...
namespace biggerprint
{
    public partial class Main : Form
    {
        ...
        public PointF ToView(double x, double y)
        {
        ...
        public PointF FromView(PointF p)
        {
            var v = view.Clone();
            v.Invert();
            var ps = new PointF[] { p };
        ...
        private void p_view_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (operation != null)
                operation.OnMouseMove(mouseX, mouseY, e.X, e.Y);
        ...
        {
            var point = FromView(new PointF(mouseX, mouseY));
            if (selectedElement == null)
                throw null;
            operation = new OperationScale { element = selectedElement, firstPoint = point };
